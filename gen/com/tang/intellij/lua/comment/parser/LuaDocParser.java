...
public class LuaDocParser implements PsiParser, LightPsiParser {
  ...
  /* ********************************************************** */
  // ID
  public static boolean class_or_global_name_ref(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "class_or_global_name_ref")) return false;
    if (!nextTokenIs(b, ID)) return false;
    boolean r;
    Marker m = enter_section_(b);
    r = consumeToken(b, ID);
    exit_section_(b, m, CLASS_OR_GLOBAL_NAME_REF, r);
    return r;
  }
  ...
  public static boolean generic_parameter_list(PsiBuilder b, int l) {
    ...
    p = r; // pin = 2
    r = r && report_error_(b, generic_parameter_list_2(b, l + 1));
    r = p && consumeToken(b, GT) && r;
    exit_section_(b, l, m, r, p, null);
    return r || p;
  }
  ...
  // (',' generic_parameter)*
  private static boolean generic_parameter_list_2(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "generic_parameter_list_2")) return false;
    while (true) {
      int c = current_position_(b);
      if (!generic_parameter_list_2_0(b, l + 1)) break;
      if (!empty_element_parsed_guard_(b, "generic_parameter_list_2", c)) break;
    }
    return true;
  }

  // ',' generic_parameter
  private static boolean generic_parameter_list_2_0(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "generic_parameter_list_2_0")) return false;
    boolean r;
    Marker m = enter_section_(b);
    r = consumeToken(b, COMMA);
    r = r && generic_parameter(b, l + 1);
  ...
  /* ********************************************************** */
  // (TAG_NAME_CLASS|TAG_NAME_MODULE) ID (generic_parameter_list)? (EXTENDS class_name_ref_list)? comment_string?
  ...
  private static boolean tag_class_2_0(PsiBuilder b, int l) {
    ...
    boolean r;
    Marker m = enter_section_(b);
    r = generic_parameter_list(b, l + 1);
    exit_section_(b, m, null, r);
    return r;
  }
  ...
  /* ********************************************************** */
  // TAG_NAME_REFER class_or_global_name_ref SHARP ID
  public static boolean tag_refer(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "tag_refer")) return false;
    if (!nextTokenIs(b, TAG_NAME_REFER)) return false;
    boolean r, p;
    Marker m = enter_section_(b, l, _NONE_, TAG_REFER, null);
    r = consumeToken(b, TAG_NAME_REFER);
    p = r; // pin = 1
    r = r && report_error_(b, class_or_global_name_ref(b, l + 1));
    r = p && report_error_(b, consumeTokens(b, -1, SHARP, ID)) && r;
    exit_section_(b, l, m, r, p, null);
    return r || p;
  }
  ...
  /* ********************************************************** */
  // TAG_NAME_SEE class_or_global_name_ref (SHARP ID)?
